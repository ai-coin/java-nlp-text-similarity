...
public final class TextSimilarity {
  ...
  // the idDictionary, id --> text
  private Map<String, String> idDictionary = new HashMap<>();

  // theTextDictionary, text --> id
  private Map<String, String> textDictionary = new HashMap<>();
   ...
   */
  public TextSimilarity(final Tokenizer tokenizer) {
   ...
   */
  @SuppressWarnings("WeakerAccess")
  public List<String> getSimilarDocuments(
          final String id,
          final int limit) {
  ...
  public String getText(final String id) {
    //Preconditions
    assert id != null && !id.isEmpty() : "id must be a non-empty character string";

    return idDictionary.get(id);
  }

  /**
   * Gets the ID given the text.
   *
   * @param text the ID
   * @return the text
   */
  public String getID(final String text) {
    //Preconditions
    assert text != null && !text.isEmpty() : "text must be a non-empty character string";

    return textDictionary.get(text);
  }
